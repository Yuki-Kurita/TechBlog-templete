...
if (isset($_POST["signUp"])) {
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["password2"]) && $_POST["password"] === $_POST["password2"]) {
        ...
        // ユーザIDとパスワードが入力されていたら認証->エラー処理
        try {
            $pdo = new PDO(DSN,DB_USER,DB_PASSWORD);
            // フォームデータの挿入
            $stmt = $pdo->query("SELECT id FROM administrator LIMIT 1");
            $result = $stmt->fetch();
            $id = $result['id']+1;
            $st = $pdo->prepare("INSERT INTO administrator(id, name, password) VALUES (?, ?, ?)");
            $st->execute(array($id, $username, password_hash($password, PASSWORD_DEFAULT)));  // パスワードのハッシュ化を行う（今回は文字列のみなのでbindValue(変数の内容が変わらない)を使用せず、直接excuteに渡しても問題ない）
