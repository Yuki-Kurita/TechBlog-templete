  ...
  <div class="row">
    ...
    <div class="col-md-9">
      ...
      <div class="box">
        ...
        <p class="post-title"><a href="javascript:formtitle<?php echo $post['no']?>.submit()"><?php echo $post['title'];?></a></p>
      </form>
        <p class="post-time"><?php echo $post['time'];?></p>
        <p class="post-content"><?php echo $post['description'];?></p>
        <form method="post" name="formbtn<?php echo $post['no'];?>" action="view.php">
        <input type="hidden" name="id" value="<?php echo $post['no'];?>">
        <a href="javascript:formtitle<?php echo $post['no']?>.submit()" class="square_btn">Read more</a>
      </form>
      </div>
    <?php }?>
    </div>
...
<div class="col-md-3">
  <div class="new-post"><ul>
    <?php foreach($posts as $post){ ?>
    <form method="post" name="form<?php echo $post['no'];?>" action="view.php">
    <input type="hidden" name="id" value="<?php echo $post['no'];?>">
    <li><a href="javascript:form<?php echo $post['no']?>.submit()"><?php echo $post['title'];?></a></li>
    </form>
  <?php }?>
    </ul></div>
